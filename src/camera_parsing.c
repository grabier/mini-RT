...
/*   Updated: 2025/09/25 18:49:48 by gmontoro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_parse_camera(char **sp, t_parse *program)
{
	...
	if (!ft_init_cam_origin(program, ft_atod(sp_aux[0]), ft_atod(sp_aux[1]),
			ft_atod(sp_aux[2])))
		return (printf("Error\nviewpoint\n"), ft_free(sp_aux), ft_free(sp), 0);
	sp_aux2 = ft_split(sp[2], ',');
	if (ft_count_list(sp_aux2) != 3)
		return (printf("Error\ncam vector\n"), ft_free(sp_aux),
			ft_free(sp_aux2), ft_free(sp), 0);
	if (!ft_init_cam_dir(program, ft_atod(sp_aux2[0]), ft_atod(sp_aux2[1]),
			ft_atod(sp_aux2[2])))
...
void	ft_init_viewport(t_parse *p)
{
	...
	p->cam->vp_w = p->cam->vp_h * ((double)MIN_W / (double)MIN_H);
...
int	ft_init_cam_origin(t_parse *program, double x, double y, double z)
{
	...
	program->cam->as_ratio = (double)(MIN_W / MIN_H);
	return (1);
}
